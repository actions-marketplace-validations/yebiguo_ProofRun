...
Every stored result is signed (HMAC-SHA256) with a random key generated on first use and kept at `.proofrun/secret` — created automatically, never committed (ProofRun makes a best-effort attempt to keep `.proofrun/` out of your repo's git history even if your own `.gitignore` never mentions it), never sent anywhere. A hand-edited `receipt.json` — even one with a fingerprint that matches perfectly — no longer verifies, and reads as `NOT RUN` instead of the value someone wrote into it.
